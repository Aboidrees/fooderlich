...
class MockFooderlichService {
  ...
  Future<List<ExploreRecipe>> _getTodayRecipes() async {
    ...
    final dataString = await _loadAsset(Assets.sampleDataSampleExploreRecipes);
    // Decode to json
  ...
  Future<List<Post>> _getFriendFeed() async {
    ...
    await Future.delayed(const Duration(milliseconds: 1000));
    // Load json from file system
    final dataString = await _loadAsset(Assets.sampleDataSampleFriendsFeed);
    // Decode to json
    final Map<String, dynamic> json = jsonDecode(dataString);
  ...
  Future<List<SimpleRecipe>> getRecipes() async {
    ...
    await Future.delayed(const Duration(milliseconds: 1000));
    // Load json from file system
    final dataString = await _loadAsset(Assets.sampleDataSampleRecipes);
    // Decode to json
    final Map<String, dynamic> json = jsonDecode(dataString);
